...
function exportCSV(logs: AttendanceLog[], absentEmps: EmployeeWithUser[], date: string) {
  const rows = [
    ['Employee', 'Employee ID', 'Department', 'Date', 'Scheduled Start', 'Scheduled End',
     'Check In', 'Check Out', 'Late (min)', 'OT (min)', 'Status', 'Method'],
    ...logs.map(l => [
    ...
    ...absentEmps.map(e => [
      e.fullName, e.employeeId, e.department ?? '', date, e.shiftStart, e.shiftEnd,
      '', '', '', '', 'absent', '',
    ]),
  ];
...
export function WorkforceAttendanceView() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [logsRes, empsRes] = await Promise.all([
      ...
      WorkforceController.getEmployees({ status: 'active' }),
    ]);
  ...
  useEffect(() => {
    const channel = supabase
      .channel('workforce-attendance-view')
  ...
  const absentEmps    = employees.filter(e => !checkedInIds.has(e.employeeId));
  ...
  const filteredAbsent = absentEmps.filter(e =>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => exportCSV(logs, absentEmps, filterDate)}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <StatCard label="Late" value={lateLogs.length} icon={AlertCircle}
        ...
        <StatCard label="Absent" value={absentEmps.length} icon={UserX}
          colorText={absentEmps.length > 0 ? 'text-orange-700' : 'text-gray-500'}
          colorBg={absentEmps.length > 0 ? 'bg-orange-50' : 'bg-gray-50'}
          colorIcon={absentEmps.length > 0 ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-400'} />
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <input
          ...
          className="px-3 py-2.5 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-amber-500"
        />
        <button
        ...
        <button onClick={loadData} className="p-2.5 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors bg-white">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 text-left">
                  {['Employee', 'Department', 'Shift', 'Check In', 'Check Out', 'Late', 'Overtime', 'Status'].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredLogs.map(log => (
                  <tr key={log.id} className="hover:bg-gray-50/80 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-sm">
                      {log.lateMinutes > 0
                        ? <span className="text-red-600 font-semibold">{log.lateMinutes}m</span>
                        : <span className="text-gray-400">—</span>}
                    </td>
                    <td className="px-5 py-3.5 text-sm">
                      {log.overtimeMinutes > 0
                        ? <span className="text-blue-600 font-medium">+{log.overtimeMinutes}m</span>
                        : <span className="text-gray-400">—</span>}
                    </td>
                    <td className="px-5 py-3.5">
                ...
                {showAbsent && filteredAbsent.map(emp => (
                  <tr key={emp.id} className="hover:bg-red-50/30 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-sm text-gray-400">{emp.department ?? '—'}</td>
