...
import { 
  ...
  Search, Info, CheckCircle2, ShoppingBag 
} from 'lucide-react';
...
export function CustomerMenuView() {
  ...
  const categories = useMemo(() => {
    ...
    return [{ id: 'All', name: 'All' }, ...orderedCategories.map(cat => ({ id: cat.id, name: cat.name }))];
  }, [menuCategories]);
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      ...
      const isVisible = product.isActive !== false && (product.availabilityStatus || 'available') === 'available';
  ...
  const addToCart = (product: Product) => {
    setCartItems(current => {
      ...
      if (existingNew) {
        return current.map(item => item.id === existingNew.id ? { ...item, quantity: item.quantity + 1 } : item);
      }
      return [...current, {
  ...
  const updateQuantity = (cartItemId: string, change: number) => {
    setCartItems(current => current.map(item => {
      if (item.id !== cartItemId || item.source === 'existing') return item;
  ...
  const handleSendToKitchen = async (targetTableId?: string) => {
    ...
    if (!tableIdToSend) { setShowTableModal(true); return; }
    ...
    try {
      ...
      if (!orderId) {
        ...
        await supabase.from('orders').insert([{ 
          id: orderId, table_id: tableIdToSend, table_number: result.order.tableNumber,
  ...
  return (
    <div className="flex h-screen w-full bg-[#EAEEF3] overflow-hidden relative">
      ...
      {/* ─── SUCCESS OVERLAY (IN-SYSTEM CONFIRMATION) ─── */}
      {showSuccess && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center animate-in fade-in duration-300">
          <div className="absolute inset-0 bg-white/20 backdrop-blur-[2px]" />
          <div className="bg-orange-500 text-white px-10 py-8 rounded-[40px] shadow-2xl flex flex-col items-center gap-4 scale-110 border-4 border-white animate-in zoom-in duration-300">
            <div className="bg-white rounded-full p-3 shadow-inner">
              <CheckCircle2 className="size-12 text-orange-500" strokeWidth={3} />
            </div>
            <div className="text-center">
              <h2 className="text-2xl font-black uppercase tracking-tighter">Order Sent</h2>
              <p className="text-orange-100 font-bold text-sm">Kitchen is notified</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {filteredProducts.map((product) => (
              <div key={product.id} onClick={() => addToCart(product)} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all cursor-pointer border-2 border-transparent active:scale-95 group flex flex-col">
                <div className="relative h-32 bg-gray-50">
                  <img src={product.image || 'https://via.placeholder.com/300x200?text=No+Image'} alt={product.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                </div>
                <div className="p-3 flex flex-col flex-1">
                  <h3 className="text-sm font-bold text-gray-800 line-clamp-2 min-h-[2.5rem] mb-2">{product.name}</h3>
        ...
        <div className="bg-white border-t p-3 flex gap-2 overflow-x-auto no-scrollbar shadow-[0_-4px_10px_rgba(0,0,0,0.03)]">
          {categories.map((cat) => (
            <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className={`px-8 py-4 rounded-2xl font-black whitespace-nowrap transition-all text-sm uppercase tracking-wider ${selectedCategory === cat.id ? 'bg-orange-500 text-white shadow-lg shadow-orange-200 -translate-y-1' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
              {cat.name}
            </button>
          ))}
        </div>
      ...
      <aside className="w-[380px] bg-white border-l flex flex-col shadow-xl z-10">
        <div className="p-6 bg-orange-500 text-white shadow-lg">
          <div className="flex justify-between items-start mb-2">
            <div><p className="text-orange-100 text-[10px] font-bold uppercase tracking-widest">Order Detail</p><h2 className="text-2xl font-black">{selectedTableLabel}</h2></div>
            <button onClick={() => setShowTableModal(true)} className="bg-orange-400 p-2 rounded-xl hover:bg-orange-600 transition-colors"><Plus className="size-5" /></button>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
          {cartItems.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-300 italic"><ShoppingCart className="size-12 mb-2 opacity-20" /><p>Cart is empty</p></div>
          ) : (
            cartItems.map((item) => (
              <div key={item.id} className="bg-gray-50 rounded-2xl p-4 border border-gray-100 flex items-center gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {item.source === 'existing' && <CheckCircle2 className="size-3 text-emerald-500" />}
                  </div>
                ...
                {item.source === 'new' ? (
                  <div className="flex items-center bg-white rounded-xl border p-1 gap-3 shadow-sm">
                    <button onClick={() => updateQuantity(item.id, -1)} className="p-1.5 hover:bg-gray-100 rounded-lg text-orange-500"><Minus className="size-4" strokeWidth={3} /></button>
                    <span className="font-black text-gray-800 w-4 text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.id, 1)} className="p-1.5 hover:bg-gray-100 rounded-lg text-orange-500"><Plus className="size-4" strokeWidth={3} /></button>
                  </div>
                ) : (
                  <div className="bg-emerald-100 text-emerald-700 px-2 py-1 rounded-lg text-[10px] font-bold uppercase">Ordered ×{item.quantity}</div>
                )}
              </div>
            ))
          )}
        ...
        <div className="p-6 border-t bg-gray-50 space-y-4">
          ...
          <button onClick={() => handleSendToKitchen()} disabled={cartItems.filter(i => i.source === 'new').length === 0 || isSending} className="w-full bg-orange-500 hover:bg-orange-600 disabled:bg-gray-200 text-white font-black py-4 rounded-2xl shadow-lg shadow-orange-200 transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-sm">
            {isSending ? 'Sending...' : <><ShoppingBag className="size-5" /> Send to Kitchen</>}
          </button>
        </div>
      </aside>
        ...
        <div className="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-6 backdrop-blur-sm">
          <div className="bg-white rounded-[40px] w-full max-w-2xl p-8 shadow-2xl">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {tables.map(table => (
                <button key={table.id} onClick={() => { setTableForSend(table.id); setSelectedTableId(table.id); setShowTableModal(false); }} className={`p-6 rounded-3xl border-2 text-left transition-all ${table.status === 'available' ? 'border-emerald-100 bg-emerald-50/50' : 'border-orange-100 bg-orange-50/50'}`}>
