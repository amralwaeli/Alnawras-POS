...
export function ReportsView() {
  ...
  const totalItems = completed.reduce((s, o) => s + o.items.reduce((ss, i) => ss + i.quantity, 0), 0);
    ...
    <div className="h-full overflow-y-auto bg-gray-50">
      <div className="p-6 space-y-6 max-w-5xl">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Total Revenue', value: `SAR ${totalRevenue.toFixed(2)}`, icon: DollarSign, bg: 'bg-emerald-50', ic: 'bg-emerald-100 text-emerald-600', vc: 'text-emerald-700' },
            { label: 'Total Orders', value: orders.length, icon: ShoppingCart, bg: 'bg-blue-50', ic: 'bg-blue-100 text-blue-600', vc: 'text-blue-700' },
            { label: 'Tax Collected', value: `SAR ${totalTax.toFixed(2)}`, icon: TrendingUp, bg: 'bg-purple-50', ic: 'bg-purple-100 text-purple-600', vc: 'text-purple-700' },
              ...
              <tbody className="divide-y divide-gray-50">
                {orders.map(o => (
                  <tr key={o.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-sm font-bold text-gray-900">SAR {o.total.toFixed(2)}</td>
